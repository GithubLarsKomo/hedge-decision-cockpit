...
describe('ETF mapping review status', () => {
  it('is current before the scheduled review date', () => {
    const result = evaluateEtfMappingReviewStatus(mapping, '2026-10-29', policy);
    assert.equal(result.status, 'current');
    assert.equal(result.next_review_date, '2026-10-30');
    assert.equal(result.days_until_due, 1);
  });

  it('becomes due exactly on the scheduled review date', () => {
    const result = evaluateEtfMappingReviewStatus(mapping, '2026-10-30', policy);
    assert.equal(result.status, 'due');
    assert.equal(result.days_until_due, 0);
  });

  it('remains due through the configured grace window', () => {
    const result = evaluateEtfMappingReviewStatus(mapping, '2026-11-13', policy);
    assert.equal(result.status, 'due');
    assert.equal(result.days_until_due, -14);
  });

  it('becomes overdue after the grace window', () => {
    const result = evaluateEtfMappingReviewStatus(mapping, '2026-11-14', policy);
    assert.equal(result.status, 'overdue');
    assert.equal(result.days_until_due, -15);
  });

  it('is deterministic and carries mapping identity', () => {
    const first = evaluateEtfMappingReviewStatus(mapping, '2026-10-30', policy);
    const second = evaluateEtfMappingReviewStatus(mapping, '2026-10-30', policy);
  ...
  it('rejects invalid policy values and dates', () => {
    assert.throws(() => evaluateEtfMappingReviewStatus(mapping, '2026-10-30', { review_interval_days: 0, overdue_grace_days: 14 }));
