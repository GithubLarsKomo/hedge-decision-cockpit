...
function requiredNumber(value: string, column: string, row: number): number {
  if (!value) throw new Error(`Missing ${column} in CSV row ${row}.`);
...
export function importMarketSnapshotCsv(csv: string, options: CsvImportOptions): NormalizedMarketSnapshot[] {
  ...
  const inputs: MarketSnapshotInput[] = lines.slice(1).map((line, index) => {
    ...
    if (fields.length !== headers.length) throw new Error(`CSV row ${row} has ${fields.length} fields; expected ${headers.length}.`);
