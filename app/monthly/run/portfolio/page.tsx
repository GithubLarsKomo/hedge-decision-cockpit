...
async function savePortfolioSnapshot(formData: FormData) {
  ...
  const activePurchaseInstrument = text(formData, 'active_purchase_instrument');
  const mappedInstruments = csv(text(formData, 'mapped_instruments'));
  if (activePurchaseInstrument && !mappedInstruments.includes(activePurchaseInstrument)) {
    mappedInstruments.unshift(activePurchaseInstrument);
  }
  ...
  const payload: PortfolioSnapshotPayload = {
    ...
    exposures: [{
      exposure_id: text(formData, 'exposure_id'),
      target_weight: numberValue(formData, 'exposure_target_weight'),
      current_weight: numberValue(formData, 'exposure_current_weight'),
      gap_amount: numberValue(formData, 'exposure_gap_amount'),
      target_source: text(formData, 'target_source') as PortfolioSnapshotPayload['exposures'][number]['target_source'],
      ...(activePurchaseInstrument ? { active_purchase_instrument: activePurchaseInstrument } : {}),
      mapped_instruments: mappedInstruments,
      mapping_version: text(formData, 'mapping_version')
    }],
  ...
  return (
    <main className="mx-auto min-h-screen max-w-5xl space-y-8 px-6 py-10">
      <header className="space-y-3">
        ...
        <p className="max-w-3xl text-slate-600">Strukturierte Browser-Eingabe statt JSON. Beim Speichern werden der bestehende Snapshot-Vertrag, die kanonische SHA-256-Berechnung und der idempotente Import-Service verwendet.</p>
      </header>

      {error && (
        <section className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">
          <strong>Snapshot konnte nicht gespeichert werden.</strong>
      ...
      <form action={savePortfolioSnapshot} className="space-y-6">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div>
            <h2 className="text-lg font-semibold text-slate-950">Exposure</h2>
            <p className="mt-1 text-sm text-slate-500">Dieser erste Browser-Slice unterstützt einen Exposure-Block. Mehrfach-Exposure folgt als eigener UX-Slice.</p>
          </div>
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            <label className={labelClass}>Exposure-ID<input required name="exposure_id" placeholder="emerging-markets" className={inputClass} /></label>
            <label className={labelClass}>Zielgewicht (0–1)<input required min="0" max="1" step="0.001" type="number" name="exposure_target_weight" className={inputClass} /></label>
            <label className={labelClass}>Aktuelles Gewicht (0–1)<input required min="0" max="1" step="0.001" type="number" name="exposure_current_weight" className={inputClass} /></label>
            <label className={labelClass}>Gap EUR<input required step="0.01" type="number" name="exposure_gap_amount" className={inputClass} /></label>
            <label className={labelClass}>Zielquelle<select name="target_source" defaultValue="estimated" className={inputClass}><option value="observed">observed</option><option value="estimated">estimated</option><option value="manual">manual</option></select></label>
            <label className={labelClass}>Mapping-Version<input required name="mapping_version" defaultValue={month} className={inputClass} /></label>
            <label className={labelClass}>Aktives Kaufinstrument<input name="active_purchase_instrument" placeholder="ISIN/Ticker" className={inputClass} /></label>
            <label className={`${labelClass} md:col-span-2`}>Gemappte Instrumente, kommagetrennt<input required name="mapped_instruments" placeholder="IE00..., IE00..." className={inputClass} /></label>
          </div>
        </section>
